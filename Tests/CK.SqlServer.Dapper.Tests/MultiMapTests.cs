...
public class MultiMapTests : TestBase
{
    ...
    public void ParentChildIdentityAssociations()
    {
        ...
        var parents = controller.Query<Parent, Child, Parent>( "select 1 as [Id], 1 as [Id] union all select 1,2 union all select 2,3 union all select 1,4 union all select 3,5",
            ( parent, child ) =>
            {
...
Order by p.Id";
            ...
            var data = controller.Query<Post, User, Post>( sql, ( post, user ) => { post.Owner = user; return post; } ).ToList();
            var p = data[0];
        ...
        {
            controller.Execute( "drop table #Users drop table #Posts" );
        }
    }
    ...
    {
        controller.Execute( "create table #dog(Age int, Name nvarchar(max)) insert #dog values(1, 'Alf')" );
        try
        {
            var tuple = controller.Query<Dog, Dog, Tuple<Dog, Dog>>( "select * from #dog d1 join #dog d2 on 1=1", Tuple.Create, splitOn: "Age" ).Single();
            ...
            tuple = controller.Query<Dog, Dog, Tuple<Dog, Dog>>( "select * from #dog d1 join #dog d2 on 1=1", Tuple.Create, splitOn: "Age" ).Single();
        ...
        {
            controller.Execute( "drop table #dog" );
        }
    }
    ...
    public void TestReadMultipleIntegersWithSplitOnAny()
    {
        Assert.Equal(
            ...
            controller.Query<int, int, int, Tuple<int, int, int>>( "select 1,2,3 union all select 4,5,6", Tuple.Create, splitOn: "*" )
        );
    }
...
Order by p.Id";
            ...
            var grid = controller.QueryMultiple( sql );
        ...
        {
            controller.Execute( "drop table #Users drop table #Posts drop table #Comments" );
        }
    }
    ...
    {
        var result = controller.Query<Foo1, Bar1, Tuple<Foo1, Bar1>>( "select 1 as Id, 2 as BarId, 3 as BarId, 'a' as Name", Tuple.Create, splitOn: "BarId" ).First();
...
Order by p.Id";
        ...
        var data = controller.Query<dynamic, dynamic, dynamic>( sql, ( post, user ) => { post.Owner = user; return post; } ).ToList();
        var p = data[0];
    ...
    public void TestMultiMapWithSplit() // https://stackoverflow.com/q/6056778/23354
    {
        ...
        var product = controller.Query<Product, Category, Product>( sql, ( prod, cat ) =>
        {
            prod.Category = cat;
            return prod;
        } ).First();
    ...
    public void TestMultiMappingVariations()
    {
        ...
        var order = controller.Query<dynamic, dynamic, dynamic, dynamic>( sql, ( o, owner, creator ) => { o.Owner = owner; o.Creator = creator; return o; } ).First();
        ...
        order = controller.Query<dynamic, dynamic, dynamic, dynamic, dynamic>( sql, ( o, owner, creator, address ) =>
        {
            o.Owner = owner;
            o.Creator = creator;
        ...
        order = controller.Query<dynamic, dynamic, dynamic, dynamic, dynamic, dynamic>( sql, ( a, b, c, d, e ) => { a.B = b; a.C = c; a.C.D = d; a.E = e; return a; } ).First();
    ...
    public void TestMultiMapWithConstructor()
    {
        ...
        try
        {
            const string sql = @"select * from #Posts p 
                           left join #Users u on u.Id = p.OwnerId 
                           Order by p.Id";
            PostWithConstructor[] data = controller.Query<PostWithConstructor, UserWithConstructor, PostWithConstructor>( sql, ( post, user ) => { post.Owner = user; return post; } ).ToArray();
            var p = data.First();
...
";
            ...
            var data = controller.Query<ReviewBoard>( sql, types, mapper ).ToList();
...
@"select 
    ...
    ";
        var personWithAddress = controller.Query<Person, Address, Extra, Tuple<Person, Address, Extra>>
    ...
    public void TestMultiMappingWithNonReturnedProperty()
    {
        ...
        var postWithBlog = controller.Query<Post_DupeProp, Blog_DupeProp, Post_DupeProp>( sql,
            ( p, b ) =>
            {
                p.Blog = b;
                return p;
    ...
    public void TestInvalidSplitCausesNiceError()
    {
        try
        {
            controller.Query<User, User, User>( "select 1 A, 2 B, 3 C", ( x, y ) => x );
        }
        ...
        try
        {
            controller.Query<dynamic, dynamic, dynamic>( "select 1 A, 2 B, 3 C", ( x, y ) => x );
        }
        catch( ArgumentException )
        {
            // expecting an app exception due to multi mapping being bodged 
