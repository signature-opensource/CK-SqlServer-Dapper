...
namespace CK.SqlServer.Dapper.Tests
{
    public class ConstructorTests : TestBase
    {
        ...
        {
            var order = controller.Query<AbstractInheritance.ConcreteOrder>( "select 1 Internal,2 Protected,3 [Public],4 Concrete" ).First();
        ...
        {
            MultipleConstructors mult = controller.Query<MultipleConstructors>( "select 0 A, 'Dapper' b" ).First();
        ...
        {
            ConstructorsWithAccessModifiers value = controller.Query<ConstructorsWithAccessModifiers>( "select 0 A, 'Dapper' b" ).First();
        ...
        public void TestNoDefaultConstructor()
        {
            ...
            NoDefaultConstructor nodef = controller.Query<NoDefaultConstructor>( "select CAST(NULL AS integer) A1,  CAST(NULL AS integer) b1, CAST(NULL AS real) f1, 'Dapper' s1, G1 = @id", new { id = guid } ).First();
        ...
        {
            NoDefaultConstructorWithEnum nodef = controller.Query<NoDefaultConstructorWithEnum>( "select cast(2 as smallint) E1, cast(5 as smallint) n1, cast(null as smallint) n2" ).First();
        ...
        {
            var rows = controller.Query<_ExplicitConstructors>( @"
...
SELECT * FROM @ExplicitConstructors"
        ...
        {
            var output = controller.Query<WithPrivateConstructor>( "select 1 as Foo" ).First();
